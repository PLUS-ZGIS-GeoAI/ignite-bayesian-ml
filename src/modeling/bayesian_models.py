...
def create_model_ffmc_adjustment_aspect(
    X: pd.DataFrame, y: pd.Series, coords: dict
) -> pm.Model:
...
def create_model_ffmc_adjustment_foresttype(
    X: pd.DataFrame, y: pd.Series, coords: dict
) -> pm.Model:
    with pm.Model(coords=coords) as model:  # type: ignore
...
def create_model_ffmc_adjustment_canopy_cover(
    X: pd.DataFrame, y: pd.Series, coords: dict
) -> pm.Model:
    with pm.Model(coords=coords) as model:  # type: ignore
        canopy_cover_groups_idx = pm.MutableData(
            "canopy_cover_groups_idx", X["canopy_cover_categorized"]
        )
        ffmc = pm.MutableData("ffmc", X.ffmc)
        fire_labels = pm.MutableData("fire", y)
        ...
        intercept = pm.Cauchy("intercept", 0, 1)
        beta_ffmc = pm.Cauchy(
            "beta_ffmc", mu_b1, sigma_b1, dims=("canopy_cover_groups")
        )
        error_var = pm.Cauchy("error_beta", 0, 1)
...
def create_model_ffmc_adjustment_exp1(
    X: pd.DataFrame, y: pd.Series, coords: dict
) -> pm.Model:
    with pm.Model(coords=coords) as model:  # type: ignore
        ...
        beta_canopy_cover = pm.Cauchy(
            "beta_canopy_cover", 0, 1, dims=("canopy_cover_groups")
        )
        error_var = pm.Cauchy("error_beta", 0, 1)
...
def create_model_ffmc_adjustment_exp2(
    X: pd.DataFrame, y: pd.Series, coords: dict
) -> pm.Model:
    with pm.Model(coords=coords) as model:  # type: ignore
        aspect = pm.MutableData("aspect", X["aspect_categorized"])
        ...
        mu_asp, sigma_asp = pm.Cauchy("mu_b1", 0.0, 1.0), pm.Exponential("sigma_b1", 1)
        mu_ft, sigma_ft = pm.Cauchy("mu_b2", 0.0, 1.0), pm.Exponential("sigma_b2", 1)
        mu_cc, sigma_cc = pm.Cauchy("mu_b3", 0.0, 1.0), pm.Exponential("sigma_b3", 1)

        # specify priors for the features
        intercept = pm.Cauchy("intercept", 0, 1)
        beta_aspect = pm.Cauchy(
            "beta_aspect", mu_asp, sigma_asp, dims=("aspect_groups")
        )
        beta_foresttype = pm.Cauchy(
            "beta_foresttype", mu_ft, sigma_ft, dims=("foresttype_groups")
        )
        beta_canopy_cover = pm.Cauchy(
            "beta_canopy_cover", mu_cc, sigma_cc, dims=("canopy_cover_groups")
        )
        error_var = pm.Cauchy("error_beta", 0, 1)
...
def create_st_blr(
    ...
    with pm.Model(coords=coords) as model:  # type: ignore
        ...
        forestroad_density = pm.MutableData("forestroad_density", X.forestroad_density)
        ...
        farmyard_density = pm.MutableData("farmyard_density", X.farmyard_density)
        ...
        beta_slope = pm.Cauchy(
            "beta_slope", mu_b2, sigma_b2, dims=("spatial_groups", "temporal_groups")
        )
        beta_aspect = pm.Cauchy(
            "beta_aspect",
            mu_b3,
        ...
        mean = (
            ...
            + beta_forestroad_density[spatial_groups_idx, temporal_groups_idx]
            * forestroad_density
            + beta_railway_density[spatial_groups_idx, temporal_groups_idx]
            * railway_density
            + beta_hikingtrail_density[spatial_groups_idx, temporal_groups_idx]
            * hikingtrail_density
            + beta_farmyard_density[spatial_groups_idx, temporal_groups_idx]
            * farmyard_density
...
def create_blr_v2(X: pd.DataFrame, y: pd.Series, coords: dict) -> pm.Model:
    with pm.Model(coords=coords) as model:  # type: ignore
        # data containers
        elevation = pm.MutableData("elevation", X.elevation_encoded)
        slope = pm.MutableData("slope", X.slope_encoded)
...
def create_st_intercept_blr_v2(
    X: pd.DataFrame,
    y: pd.Series,
    coords: dict,
    spatial_grouping_variable: str,
